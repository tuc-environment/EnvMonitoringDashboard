...
class HttpClient {
  ...
  public async getUnassignedSensors(params?: {
    stationID?: number
    offset?: number
    limit?: number
  }): Promise<Response<Sensor[]> | null> {
    ...
    const resp = await this.get<Sensor[]>(`/sensors/unassigned?${ret.join('&')}`)
    return resp
  }
  ...
  public async batchUpsertSensor(sensors: Sensor[]): Promise<Response<Sensor> | null> {
    const resp = await this.post<Sensor>('/sensors/batch', sensors)
    return resp
  }
