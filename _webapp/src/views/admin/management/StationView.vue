<template>
  <div class="main-content">
    ...
    <div v-if="!requesting">
      <table-paginator :offset="offset" :limit="limit" :total="total" @to-previous="toPreviousPage" @to-index="toOffset"
        @to-next="toNextPage" />
      <div class="card">
          ...
          <tbody>
            <tr v-for="station in allStations" :key="station.id">
              ...
              <td>
                ...
                <div v-else>
                  <button type="button" class="btn btn-outline-success btn-sm mx-2" @click="viewSensorData(station.id)">
                    查看所有传感器
                  </button>
                  ...
                  <button type="button" class="btn btn-outline-danger btn-sm mx-2" @click="onDeleteStation(station.id)">
                    删除
                  </button>
                </div>
              </td>
    ...
    <upsert-station-modal ref="upsertStationModal" :visible="isShowingStationUpsertModal" title="创建站点"
      @did-upsert-station="onStationUpserted" @close="onModalClosed" />
    <double-confirm-modal :visible="isShowingDeletionModal" :title="'删除站点 ' + operatingStationId" content="确定删除站点？"
      @close="onDeleteStationModalClosed" @on-confirm="confirmDeletingStation" />
  </div>
</template>
