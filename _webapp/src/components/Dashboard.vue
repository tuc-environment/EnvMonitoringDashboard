<template>
    <div class="dashboardContainer">
        <div class="leftPanel d-flex flex-column justify-content-between">
            ...
            <LineChart :station="dashboardStore.$state.treeStationSelected" :sensors="dashboardStore.$state.treeSensorSelected ? [dashboardStore.$state.treeSensorSelected] : []
                " :records="dashboardStore.$state.treeSensorRecordsLoaded" title="数据对比" default-text="请选择数据项"
                no-data-text="暂无数据" />
        </div>
        <div class="centerSpace"></div>
        <div class="rightPanel d-flex flex-column justify-content-between">
            <LineChart :station="selectedStation" :sensors="airRelatedSensors" :records="airRelatedRecords" title="空气参数"
                default-text="请选择站点" no-data-text="暂无数据" />
            <LineChart :station="selectedStation" :sensors="soilRelatedSensors" :records="soilRelatedRecords" title="土壤参数"
                default-text="请选择站点" no-data-text="暂无数据" />
        </div>
    </div>
</template>
    ...
    if (stationId) {
        ...
        if (sensors.length > 0) {
            const airSensorIds = sensors
                ? (sensors.filter((sensor) => sensor.id && sensor.name && airOptionNames.includes(sensor.name)).map((sensor) => sensor.id) as number[])
                : []
            const soilSensorIds = sensors
                ? (sensors.filter((sensor) => sensor.id && sensor.name && soilOptionNames.includes(sensor.name)).map((sensor) => sensor.id) as number[])
                : []
            ...
            soilRelatedRecords.value = records.filter((record) => soilSensorIds.includes(record.sensor_id))
